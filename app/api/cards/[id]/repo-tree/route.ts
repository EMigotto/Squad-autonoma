...
export async function GET(
  ...
  try {
    ...
    const { data: card } = await svc
      .from("cards")
      .select("working_branch, feature:features(slug, github_repo, environment_id)")
      .eq("id", params.id)
      .single();
    ...
    // Resolve a branch: working_branch do card → branch do ambiente → main
    let branch = (card as any).working_branch as string | null;
