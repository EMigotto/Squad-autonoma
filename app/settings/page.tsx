...
export default function SettingsPage() {
  ...
  const [tab, setTab] = useState<
    "people" | "knowledge" | "agents" | "workflow" | "corp" | "skills"
  >("people");
  ...
  async function handleRedeployAll() {
    if (
      !confirm(
        "Redeploy em massa: re-sincroniza TODOS os agentes deste time com o Console, " +
        "aplicando os prompts mais recentes (incluindo a tradução pt-BR). Continuar?"
      )
    )
      return;
    setRedeploying(true);
    try {
      const res = await fetch("/api/admin/redeploy-agents", {
        ...
        body: JSON.stringify({ refreshBuiltins: true }),
      });
      const data = await res.json();
      if (!res.ok) {
        alert("erro: " + (data.error ?? res.status));
        ...
        const lines = (data.results ?? [])
          .map((r: any) => `• ${r.role}: ${r.action}`)
          .join("\n");
        alert(`Redeploy concluído:\n\n${lines}`);
        await reloadAgents();
      }
    } catch (e) {
      alert("erro: " + String(e));
    ...
    <main className="min-h-screen p-8">
      <div className="max-w-5xl mx-auto space-y-12">
        ...
        <section>
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                disabled={redeploying || agents.length === 0}
                title="Re-sincroniza todos os agentes deste time com o Console, aplicando os prompts mais recentes (inclui tradução pt-BR)"
                className="border border-development text-development px-3 py-1.5 text-xs font-semibold hover:bg-development/10 disabled:opacity-50 transition-colors"
              >
                {redeploying ? "redeployando…" : "↻ redeploy em massa"}
              </button>
              <button
                onClick={() => {
                  setCreatingNew(true);
          ...
          <div className="text-[11px] text-ink-500 mb-4 leading-relaxed">
            Use <strong>↻ redeploy em massa</strong> após atualizar prompts no código
            (ex.: tradução pt-BR) para propagar a todos os agentes já provisionados deste time
            de uma vez — sem precisar editar um por um.
          </div>
