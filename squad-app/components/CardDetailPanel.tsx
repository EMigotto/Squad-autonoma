...
export default function CardDetailPanel({
  ...
  async function handleReject(reason: string) {
    setActionLoading(true);
    // Se há gate, usa o endpoint do gate; senão, decide pelo card (fallback
    // quando o registro de gate está ausente).
    const url = detail?.gate
      ? `/api/gates/${detail.gate.id}/decide`
      : `/api/cards/${cardId}/decide`;
    const res = await fetch(url, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  // Permite revisar sempre que o card está aguardando revisão. Se há um gate
  // atribuído, respeita o dono; se NÃO há gate (registro ausente/atrasado),
  // libera a revisão mesmo assim — os botões não devem sumir só por falta do
  // registro de gate.
  const isMineToReview = gate
    ? gate.assignee_id === currentUser.id || !gate.assignee_id
    : true;
  const isTerminal =
    card.status === "done" ||
      ...
      <div className="fixed inset-0 bg-ink-950/80 flex items-stretch justify-end z-50">
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-5">
            ...
            {/* Rodar aplicação (preview) + verificar integração */}
            <RunAndVerifyPanel cardId={cardId} feature={feature} />
...
// Painel: rodar a aplicação (preview p/ o PM navegar) + verificar integração
// (roda build/test do código combinado antes de concluir).
function RunAndVerifyPanel({ cardId, feature }: { cardId: string; feature: any }) {
  const [previewMsg, setPreviewMsg] = useState("");
  const [previewUrl, setPreviewUrl] = useState<string | null>(feature?.preview_url ?? null);
  const [previewing, setPreviewing] = useState(false);
  const [intMsg, setIntMsg] = useState("");
  const [checking, setChecking] = useState(false);

  async function runPreview() {
    setPreviewing(true); setPreviewMsg("");
    try {
      const res = await fetch(`/api/cards/${cardId}/preview`, { method: "POST" });
      const d = await res.json();
      if (!res.ok) { setPreviewMsg(d.error ?? "falha"); }
      else if (d.status === "sem_deploy_hook") {
        setPreviewMsg("configure o Vercel deploy hook em Settings → workflow para habilitar o preview.");
      } else {
        setPreviewUrl(d.preview_url ?? null);
        setPreviewMsg(d.preview_url ? "preview disparado ✓" : "deploy disparado ✓ (a URL aparece no Vercel)");
      }
    } catch (e) { setPreviewMsg(String(e)); } finally { setPreviewing(false); }
  }

  async function runIntegration() {
    setChecking(true); setIntMsg("");
    try {
      const res = await fetch(`/api/cards/${cardId}/integration-check`, { method: "POST" });
      const d = await res.json();
      setIntMsg(res.ok
        ? "verificador de integração disparado — acompanhe a sessão; ele relata conflitos/erros sem alterar o código."
        : (d.error ?? "falha"));
    } catch (e) { setIntMsg(String(e)); } finally { setChecking(false); }
  }
    ...
    <div className="border border-ink-700 rounded-card bg-ink-900/50 p-4">
      ...
      <div className="flex flex-wrap gap-2">
        <button onClick={runPreview} disabled={previewing}
          className="px-3 py-1.5 text-xs font-semibold rounded-card bg-development text-ink-950 hover:opacity-90 disabled:opacity-50">
          {previewing ? "subindo…" : "▶ rodar aplicação (preview)"}
        </button>
        <button onClick={runIntegration} disabled={checking}
          className="px-3 py-1.5 text-xs font-semibold rounded-card border border-qa text-qa hover:bg-qa/10 disabled:opacity-50">
          {checking ? "verificando…" : "🔌 verificar integração"}
        </button>
        {previewUrl && (
          <a href={previewUrl} target="_blank" rel="noreferrer"
            className="px-3 py-1.5 text-xs font-semibold rounded-card border border-development text-development hover:bg-development/10">
            abrir preview →
          </a>
        )}
      </div>
      {previewMsg && <div className="mt-2 text-[11px] text-ink-300">{previewMsg}</div>}
      {intMsg && <div className="mt-1 text-[11px] text-ink-300">{intMsg}</div>}
      <p className="mt-3 text-[10.5px] text-ink-500 leading-relaxed">
