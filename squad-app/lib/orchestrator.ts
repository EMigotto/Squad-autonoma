...
export async function previewKickoff(
  ...
  if ((feature as any).feature_type === "bug" && (feature as any).bug_context) {
    const bc = (feature as any).bug_context;
    initial_message +=
      `\n\n=== CORREÇÃO DE BUG (prioridade) ===\n` +
      `Esta é uma feature de CORREÇÃO. Reproduza, identifique a causa raiz e corrija com a MENOR ` +
      `mudança possível, sem quebrar o que já funciona e seguindo os padrões do repositório.\n` +
      `Erro reportado:\n\`\`\`\n${String(bc.error ?? "").slice(0, 3000)}\n\`\`\`\n` +
      (bc.file ? `Arquivo indicado: ${bc.file}\n` : "") +
      (bc.repro ? `Como reproduzir: ${bc.repro}\n` : "") +
      ((feature as any).github_issue_number
        ? `Ao terminar, comente a correção na issue #${(feature as any).github_issue_number} (ela será fechada quando o card for concluído).\n`
        : "") +
      `===\n`;
  }
...
// ============================================================
// (a) Verificação de INTEGRAÇÃO: dispara um agente que mescla a working branch
// na base, roda install+build+test no código COMBINADO e reporta. Pega erros
// que só aparecem quando as features se juntam (integration hell).
// ============================================================
export async function runIntegrationCheck(cardId: string): Promise<{ session_id: string }> {
  const sb = createServiceClient();
  const { data: card } = await sb.from("cards").select("*").eq("id", cardId).single();
  if (!card) throw new Error("card não encontrado");
  const feature = await ensureFeatureEnvironment(card.feature_id);
  const gitId = await resolveGitIdentity((feature as any).created_by);
  const token = gitId.token;
  const base = (feature as any).source_branch || "main";
  const working = (feature as any).working_branch || "main";

  await sb.from("features").update({ integration_status: "running" }).eq("id", feature.id);

  const prompt =
    `=== IDIOMA OBRIGATÓRIO: PORTUGUÊS DO BRASIL ===\n\n` +
    `Você é o AGENTE VERIFICADOR DE INTEGRAÇÃO. Objetivo: detectar erros que só ` +
    `aparecem quando a feature '${feature.slug}' se junta ao restante do código.\n\n` +
    `PASSOS:\n` +
    `1. Clone o repositório.\n` +
    `2. Crie uma branch temporária a partir de '${base}' e faça merge de '${working}' nela ` +
    `(NÃO faça push; é só verificação local). Se houver conflito de merge, RELATE os arquivos em conflito.\n` +
    `3. Rode a instalação de dependências, o build e a suíte de testes no código COMBINADO.\n` +
    `4. Relate em pt-BR: conflitos de merge, erros de build, testes que falharam — com arquivo e linha.\n` +
    `5. NÃO altere/corrija nada agora; apenas DIAGNOSTIQUE. Encerre com um veredito claro: ` +
    `"INTEGRAÇÃO OK" ou "INTEGRAÇÃO COM PROBLEMAS" + a lista priorizada do que corrigir.\n\n` +
    `--- Credenciais ---\nRepo: ${feature.github_repo}\n` +
    `Clone URL: https://x-access-token:${token}@github.com/${feature.github_repo}.git\n` +
    `Base: ${base} · Working: ${working}\n---\n` +
    `Desabilite assinatura de commit. Configure identidade git.`;

  const sessionId = await runProjectAgentSession(feature.project_id, `integração · ${feature.slug}`, prompt);
  return { session_id: sessionId };
}

// ============================================================
// (c) PREVIEW: dispara o deploy de preview (Vercel deploy hook) da working
// branch e devolve a URL para o PM abrir e navegar — sem precisar de código.
// ============================================================
export async function triggerPreview(cardId: string): Promise<{ preview_url: string | null; status: string }> {
  const sb = createServiceClient();
  const { data: card } = await sb.from("cards").select("*").eq("id", cardId).single();
  if (!card) throw new Error("card não encontrado");
  const feature = await ensureFeatureEnvironment(card.feature_id);
  const settings = await getAppSettings(feature.project_id);
  const hook = (settings as any).vercel_deploy_hook as string | null;
  const previewBase = (settings as any).preview_base_url as string | null;

  if (!hook) {
    return { preview_url: null, status: "sem_deploy_hook" };
  }

  await sb.from("features").update({ preview_status: "building" }).eq("id", feature.id);
  try {
    // Vercel deploy hook aceita ?ref=<branch> para escolher a branch
    const working = (feature as any).working_branch || "main";
    const url = `${hook}${hook.includes("?") ? "&" : "?"}ref=${encodeURIComponent(working)}`;
    const res = await fetch(url, { method: "POST" });
    if (!res.ok) {
      await sb.from("features").update({ preview_status: "failed" }).eq("id", feature.id);
      return { preview_url: null, status: `falha no deploy hook (${res.status})` };
    }
    // a URL final do preview depende do projeto; se houver base configurada, monta por branch
    const previewUrl = previewBase
      ? previewBase.replace("{branch}", (feature as any).working_branch || "main")
      : null;
    await sb.from("features").update({
      preview_status: "ready",
      preview_url: previewUrl,
    }).eq("id", feature.id);
    return { preview_url: previewUrl, status: "disparado" };
  } catch (e) {
    await sb.from("features").update({ preview_status: "failed" }).eq("id", feature.id);
    return { preview_url: null, status: e instanceof Error ? e.message : String(e) };
  }
}
...
export async function advanceCard(
  ...
  if (nextStage === "done") {
    ...
    // Se for um BUG vinculado a uma issue do GitHub, fecha a issue.
    try {
      const { data: feat } = await sb
        .from("features")
        .select("feature_type, github_issue_number, github_repo")
        .eq("id", card.feature_id)
        .maybeSingle();
      if (feat?.feature_type === "bug" && feat.github_issue_number && feat.github_repo) {
        const token = process.env.GITHUB_TOKEN;
        if (token) {
          const repo = feat.github_repo.replace(/^https?:\/\/github\.com\//i, "").replace(/\.git$/i, "");
          await fetch(`https://api.github.com/repos/${repo}/issues/${feat.github_issue_number}`, {
            method: "PATCH",
            headers: { Authorization: `token ${token}`, Accept: "application/vnd.github+json" },
            body: JSON.stringify({ state: "closed", state_reason: "completed" }),
          });
          await fetch(`https://api.github.com/repos/${repo}/issues/${feat.github_issue_number}/comments`, {
            method: "POST",
            headers: { Authorization: `token ${token}`, Accept: "application/vnd.github+json" },
            body: JSON.stringify({ body: "✅ Bug resolvido e concluído via {V.AI.be} Cielo." }),
          });
        }
      }
    } catch (e) {
      console.error("[advanceCard] fechar issue do bug falhou", e);
    }
